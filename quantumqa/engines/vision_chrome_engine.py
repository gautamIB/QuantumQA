...
class VisionChromeEngine:
            ...
            try:
                ...
                if contexts:
                    ...
                    try:
                        for page in self.context.pages:
                            ...
                            if "aihub" in url or "instabase" in url:
                                ...
                                print(
                                    f"🔐 Found existing session on target domain: {url}"
                                )
                                break
                    except:
                        pass
                    ...
                    if not has_target_domain:
                        ...
                        print(
                            f"   1. Manually navigate to https://aihub-uat.internal.instabase.com/ in Chrome"
                        )
                        print(f"   2. Complete login process")
                        print(
                            f"   3. Then re-run this test to use authenticated session"
                        )
    ...
    async def execute_test(self, instruction_file: str) -> Dict[str, Any]:
        ...
        for i, instruction in enumerate(instructions, 1):
            ...
            try:
                ...
                ui_context_created = self.ui_context_manager.analyze_step_for_context(
                    i, instruction)

                # Check if step needs to be executed within a specific UI context
                ui_context_needed = self.ui_context_manager.check_if_step_needs_context(
                    i, instruction)
                ...
                if ui_context_needed:
                    ...
                    print(
                        f"    🎯 Step requires UI context: {ui_context_needed['search_scope']}"
                    )
                ...
                if active_contexts_summary != "No active UI contexts":
                    ...
                    print(
                        f"    📋 Active UI contexts: {active_contexts_summary}")
                ...
                success = await self._execute_action_with_vision(action_plan,
                                                                 step_number=i)
                ...
                await asyncio.sleep(
                    1.5)  # Slightly longer pause for visual confirmation
    ...
    async def _execute_action_with_vision(self, action_plan: Dict[str, Any],
        ...
        try:
            if action == "navigate":
                ...
                element_coords = await self._find_element_with_vision(
            ...
            if "Execution context was destroyed" in error_msg:
                print(
                    f"    ⚠️ Execution context destroyed (likely due to navigation), treating as success"
                )
    ...
    async def _find_element_with_vision(
        ...
        context = {
            ...
            "previous_action":
            "navigation" if step_number == 1 else "user_action"
        }

        # Add UI context information if available
        ui_context_fields = [
            "ui_context_type", "ui_context_target", "ui_context_opened_step",
            "search_scope", "context_keywords"
        ]
        ...
        try:
            ...
            if traditional_coords:
                ...
                if self._validate_coordinates_in_viewport(traditional_coords):
                    print(
                        f"    ✅ Enhanced traditional detection succeeded at ({traditional_coords.x}, {traditional_coords.y})"
                    )
            ...
            # Special handling for export buttons
            if "export" in target_lower:
                enhanced_instruction = f"Find the export button or icon that can be clicked. Look specifically for buttons with data-content='Export', export icons, or download buttons in the top navigation or toolbar areas."
            else:
            ...
            if detection_result.found and detection_result.center_coordinates:
                ...
                is_interactive = await self._validate_interactive_element(
                    coords, action_plan["action"])

                if is_interactive:
                    print(
                        f"    ✅ Vision found interactive element at ({coords.x}, {coords.y}) confidence={detection_result.confidence:.2f}"
                    )
                    return coords
                else:
                    print(
                        f"    ⚠️ Vision found non-interactive element at ({coords.x}, {coords.y}) - retrying..."
                    )
                    ...
                    if better_coords and self._validate_coordinates_in_viewport(
                            better_coords):
                        print(
                            f"    ✅ Found better interactive element at ({better_coords.x}, {better_coords.y})"
                        )
                ...
                print(
                    f"    ❌ Vision failed to find element: {detection_result.error_message or 'Element not detected'}"
                )
                self.detection_failures += 1
                return None
    ...
    async def _validate_interactive_element(self, coordinates: Coordinates,
                                            action: str) -> bool:
    ...
    async def _find_nearby_interactive_element(
        ...
        try:
            ...
            if action == 'click' and not is_export_action:
                ...
                if semantic_result:
                    print(
                        f"    ✅ Found element with semantic search: {semantic_result['selector']}"
                    )
                    from ..core.models import Coordinates
                    return Coordinates(x=int(semantic_result['x']),
                                       y=int(semantic_result['y']))
            ...
            for radius in [15, 30, 60, 120, 200]:
                ...
                if element_info:
                    print(
                        f"    🔍 Found {element_info['tag']} at radius {radius}px (id: {element_info.get('id', 'none')}, name: {element_info.get('name', 'none')})"
                    )
                    from ..core.models import Coordinates
                    return Coordinates(x=int(element_info['x']),
                                       y=int(element_info['y']))
...
"""
            ...
            if hasattr(self, 'element_detector') and hasattr(
    ...
    def _basic_normalization(self, target: str) -> List[str]:
        ...
        return list(dict.fromkeys(
            normalized))  # Remove duplicates while preserving order

    async def _try_enhanced_traditional_detection(
            self, action_plan: Dict[str, Any], original_target: str,
            normalized_targets: List[str]) -> Optional[Coordinates]:
        """Try traditional detection with AI-normalized terms."""
        try:
            action_type = action_plan["action"]

            # Special handling for export buttons
            if action_type == "click" and "export" in original_target.lower():
                print(f"    🔍 Special handling for export button detection")

                # Direct selectors for export buttons with increased timeout
                export_selectors = [{
                    "selector": "[data-content='Export']",
                    "strategy": "direct_export_data_content"
                }, {
                    "selector":
                    "[data-content='export']",
                    "strategy":
                    "direct_export_data_content_lowercase"
                }, {
                    "selector":
                    "[data-content*='Export' i]",
                    "strategy":
                    "direct_export_data_content_case_insensitive"
                }, {
                    "selector":
                    "button[data-content='Export']",
                    "strategy":
                    "direct_export_button_data_content"
                }, {
                    "selector": "*[data-content='Export']",
                    "strategy": "direct_export_any_element"
                }, {
                    "selector": "button:has-text('Export')",
                    "strategy": "direct_export_button_text"
                }, {
                    "selector": "[aria-label*='export' i]",
                    "strategy": "direct_export_aria_label"
                }, {
                    "selector": "[title*='export' i]",
                    "strategy": "direct_export_title"
                }, {
                    "selector":
                    "button:has(.fa-file-download), button:has(.icon-export)",
                    "strategy": "direct_export_icon"
                }]

                # Try each export selector with increased timeout
                for selector_info in export_selectors:
                    try:
                        selector = selector_info["selector"]
                        strategy = selector_info["strategy"]

                        print(
                            f"    🔍 Trying direct export selector: {selector}")

                        # Try to find the element with increased timeout
                        element = self.page.locator(selector).first

                        # Check if element exists and is visible with longer timeout for export buttons
                        if await element.count() > 0:
                            try:
                                if await element.is_visible(
                                        timeout=5000
                                ):  # Longer timeout for export buttons
                                    print(
                                        f"    ✅ Found export button using {strategy}: {selector}"
                                    )
                                    ...
                                    if bounding_box:
                                        ...
                                        from ..core.models import Coordinates
                                        return Coordinates(x=center_x,
                                                           y=center_y)
                            except Exception as e:
                                print(
                                    f"    ⚠️ Export button visibility check failed: {e}"
                                )
                                continue
                    except Exception as e:
                        print(f"    ⚠️ Export selector failed: {e}")
                        continue
            ...
            if action_type == "click":
                ...
                for target in normalized_targets:
                    print(
                        f"    🔍 Trying selectors for normalized term: '{target}'"
                    )

                    # Generate smart selectors for this normalized target
                    selectors = self._generate_smart_selectors(
                        target, action_plan)
                    ...
                    for selector_info in selectors:
                        try:
                            ...
                            if await element.count() > 0:
                                try:
                                    # Use longer timeout for export-related selectors
                                    timeout = 3000 if "export" in target.lower(
                                    ) else 1000
                                    if await element.is_visible(timeout=timeout
                                                                ):
                                        print(
                                            f"    ✅ Found using {strategy} for '{target}': {selector}"
                                        )
    ...
    def _generate_smart_selectors(
            self, target: str, action_plan: Dict[str,
        ...
        selectors.extend([
            {
                "selector": f"button:has-text('{target}'):not([role='tab'])",
        ...
        if any(word in target_lower
               for word in ["workspace", "sign in", "login"]):
            selectors.extend([
                {
        ...
        selectors.extend([
            {
                ...
                "strategy": "clickable_element",
                "priority": 8
            },
            {
                "selector": f"div:has-text('{target}')[role='button']",
                "strategy": "div_button",
                "priority": 9
            },
        ])
    ...
    async def _analyze_natural_viewport(self) -> None:
        ...
        try:
            ...
            print(
                f"       Viewport: {viewport_info.get('viewportWidth', 'unknown')}x{viewport_info.get('viewportHeight', 'unknown')}"
            )
            print(
                f"       Window: {viewport_info.get('windowWidth', 'unknown')}x{viewport_info.get('windowHeight', 'unknown')}"
            )
            print(
                f"       Screen: {viewport_info.get('screenWidth', 'unknown')}x{viewport_info.get('screenHeight', 'unknown')}"
            )
            print(
                f"       Device Pixel Ratio: {viewport_info.get('devicePixelRatio', 'unknown')}"
            )
            ...
            if viewport_width > 0 and viewport_height > 0:
                print(
                    f"    ✅ Working with natural viewport: {viewport_width}x{viewport_height}"
                )

                # Calculate usable area info
                if viewport_width < 1000:
                    print(
                        f"    ⚠️ Narrow viewport detected - right-side elements may be harder to find"
                    )
                if viewport_height < 600:
                    print(
                        f"    ⚠️ Short viewport detected - bottom elements may require scrolling"
                    )
                ...
                print(
                    f"    ⚠️ Could not determine viewport dimensions, using fallback assumptions"
                )
                self._current_viewport = {
                    'width': 1200,
                    'height': 800,
            ...
            self._current_viewport = {'width': 1200, 'height': 800, 'dpr': 1}

    def _validate_coordinates_in_viewport(self,
        ...
        try:
            ...
            within_width = -margin <= coordinates.x <= viewport[
                'width'] + margin
            within_height = -margin <= coordinates.y <= viewport[
                'height'] + margin

            if not within_width:
                print(
                    f"    ⚠️ X coordinate {coordinates.x} outside viewport width {viewport['width']}"
                )
            if not within_height:
                print(
                    f"    ⚠️ Y coordinate {coordinates.y} outside viewport height {viewport['height']}"
                )
        ...
        for attempt in range(3):
            try:
                ...
                if viewport_info.get('screenWidth', 0) > 0:
                    ...
                    print(
                        f"    ⚠️ Attempt {attempt + 1}: Got invalid viewport data, retrying..."
                    )
                    await asyncio.sleep(0.5)

            except Exception as e:
                print(
                    f"    ⚠️ Attempt {attempt + 1}: Viewport info failed: {e}")
                await asyncio.sleep(0.5)
    ...
    async def _take_analysis_screenshot(self,
                                        step_number: int) -> Optional[str]:
        ...
        try:
            ...
            print(
                f"       Document: {viewport_info.get('documentWidth', 'unknown')}x{viewport_info.get('documentHeight', 'unknown')}"
            )
            print(
                f"       Scroll: ({viewport_info.get('scrollX', 'unknown')}, {viewport_info.get('scrollY', 'unknown')})"
            )
    ...
    async def _click_at_coordinates(self, coordinates: Coordinates) -> bool:
        ...
        try:
            ...
            print(
                f"    🎯 Preparing to click at coordinates ({coordinates.x}, {coordinates.y})"
            )
            ...
            print(
                f"    🖱️ Clicking at vision coordinates ({coordinates.x}, {coordinates.y})"
            )
            ...
            print(
                f"       Ready state: {await self.page.evaluate('document.readyState')}"
            )
            ...
            try:
                ...
                if element_info:
                    ...
                    print(
                        f"       ID: {element_info.get('id', 'N/A') or 'None'}"
                    )
                    print(
                        f"       Class: {element_info.get('className', 'N/A') or 'None'}"
                    )
                    print(
                        f"       Type: {element_info.get('type', 'N/A') or 'None'}"
                    )
                    print(
                        f"       Name: {element_info.get('name', 'N/A') or 'None'}"
                    )
                    print(
                        f"       Placeholder: {element_info.get('placeholder', 'N/A') or 'None'}"
                    )
                    print(
                        f"       Role: {element_info.get('role', 'N/A') or 'None'}"
                    )
                    print(
                        f"       Aria-label: {element_info.get('ariaLabel', 'N/A') or 'None'}"
                    )
                    print(
            ...
            try:
                ...
                print(
                    f"    ⚠️ Page might not be fully ready, proceeding anyway")

            # 🖱️ PERFORM ACTUAL CLICK with error handling and fallback
            click_success = False
            try:
                # Method 1: Standard mouse click
                await self.page.mouse.click(coordinates.x,
                                            coordinates.y,
                                            delay=100)
            ...
            if current_url != initial_url:
                ...
                print(
                    f"    ⚠️ No page changes detected - click might not have worked"
                )
                # Take another screenshot to compare
                await self._capture_action_screenshot("post_click_check",
                                                      coordinates)
                await asyncio.sleep(0.5)
            ...
            if "Execution context was destroyed" in error_msg:
                print(
                    f"    ⚠️ Execution context destroyed (navigation occurred), treating as success"
                )
    ...
    def _resolve_credentials(self, text: str) -> str:
        ...
        def replace_credential(match):
            ...
            try:
                ...
                if len(path_parts) != 2:
                    print(
                        f"    ⚠️ Invalid credential path format: {credential_path}"
                    )
                ...
                if value is not None:
                    print(
                        f"    🔑 Resolved credential: {credential_path} -> {value}"
                    )
                    return str(value)
                else:
                    print(
                        f"    ⚠️ Could not resolve credential: {credential_path}"
                    )
                    return match.group(0)

            except Exception as e:
                print(
                    f"    ❌ Error resolving credential {credential_path}: {e}")
                return match.group(0)

        resolved_text = re.sub(pattern, replace_credential, text)
        return resolved_text

    async def _fallback_to_traditional(self, action_plan: Dict[str, Any],
                                       step_number: int) -> bool:
    ...
    async def _generate_report(self,
                               ...
                               total_test_time: float = 0.0) -> Dict[str, Any]:
        ...
        step_times = [
            r.get("timing", {}).get("total_time", 0.0) for r in results
        ]
        avg_step_time = sum(step_times) / len(
            step_times) if step_times else 0.0
        max_step_time = max(step_times) if step_times else 0.0
        min_step_time = min(step_times) if step_times else 0.0

        parse_times = [
            r.get("timing", {}).get("parse_time", 0.0) for r in results
        ]
        execute_times = [
            r.get("timing", {}).get("execute_time", 0.0) for r in results
        ]
        avg_parse_time = sum(parse_times) / len(
            parse_times) if parse_times else 0.0
        avg_execute_time = sum(execute_times) / len(
            execute_times) if execute_times else 0.0
        ...
        if self.vision_detections > 0:
            ...
            vision_success_rate = (vision_successes /
                                   self.vision_detections) * 100

        print("\n" + "=" * 60)
        print("📊 VISION-ENHANCED CHROME ENGINE REPORT")
        print("=" * 60)
        print(
            f"📈 Overall Success Rate: {success_rate:.1f}% ({successful_steps}/{total_steps})"
        )
        ...
        print(
            f"  • Estimated cost: ${llm_stats.get('estimated_cost', 0.0):.4f}")
        ...
        report = {
            ...
            "performance_statistics": {
                ...
                "average_parse_time":
                avg_parse_time,
                "average_execute_time":
                avg_execute_time,
                "steps_per_minute":
                (total_steps /
                 (total_test_time / 60)) if total_test_time > 0 else 0.0
            }
        }
