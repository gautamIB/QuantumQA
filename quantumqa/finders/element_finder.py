...
class ElementFinder:
    ...
    async def find_clickable_element(self, page: Page, target: str, context: Dict[str, Any] = None) -> Optional[Locator]:
            ...
            try:
                ...
                if await element.count() > 0 and await element.is_visible(timeout=2000):
                    ...
                    if context.get("element_type") == "dropdown":
                        ...
                        is_in_dropdown = context.get("parent_type") == "dropdown" or "dropdown" in target.lower()
                        
                        if is_in_dropdown:
                            # For dropdown items, visibility is often sufficient
                            print(f"    🎯 Dropdown context detected - returning element for specialized handling")
                            return element
                        else:
                            # For non-dropdown items, use strict validation
                            is_clickable = await self._validate_clickable_element(element)
                            if is_clickable:
                                return element
                            else:
                                print(f"    ⚠️ Element not truly clickable, continuing search...")
                                continue
        ...
        if "dropdown" in target.lower() or context.get("parent_type") == "dropdown":
            print(f"    🔄 Attempting dropdown-specific search with wait...")
            return await self._find_dropdown_item_with_wait(page, target)
        
        return None
    
    async def _find_dropdown_item_with_wait(self, page: Page, target: str) -> Optional[Locator]:
                    ...
                    for i in range(count):
                        ...
                        if await element.is_visible(timeout=1000):
                            ...
                            if is_in_dropdown:
                                ...
                                print(f"    ✅ Found dropdown item using: {selector}")
                                
                                # Try to check if it's enabled, but don't be strict
                                try:
                                    is_enabled = await element.is_enabled(timeout=500)
                                    if is_enabled:
                                        print(f"    🎯 Element appears enabled - returning as-is")
                                        return element
                                    else:
                                        print(f"    ⚠️ Element appears disabled, but trying clickable parent...")
                                        # Try to find clickable parent
                                        clickable_parent = await self._find_clickable_parent(element)
                                        if clickable_parent:
                                            print(f"    ✅ Found clickable parent for dropdown item")
                                            return clickable_parent
                                        else:
                                            print(f"    🔄 No clickable parent found, returning original element for specialized clicking")
                                            return element  # Let specialized dropdown clicking handle it
                                except Exception:
                                    print(f"    🎯 Clickability check failed, but returning element for dropdown-specific handling")
                                    return element  # Return anyway for dropdown-specific strategies
            except Exception as e:
                continue
        
        print(f"    ❌ Could not find dropdown item: '{target}' even with specialized search")
        return None
    
    async def _find_clickable_parent(self, element: Locator) -> Optional[Locator]:
        ...
        try:
            ...
            for parent_selector in parent_selectors:
                try:
                    ...
                    if await parent.is_visible(timeout=500) and await parent.is_enabled(timeout=500):
            ...
            try:
                ...
                if await element.count() > 0 and await element.is_visible(timeout=2000):
                    ...
                    if tag_name in ["input", "textarea"]:
                        print(f"    ✅ Found input field using {strategy}: {selector}")
                        return element
    ...
    def _generate_click_selectors(self, target: str, context: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if (context.get("element_type") in ["dropdown-button", "dropdown"] or 
            context.get("has_dropdown") or "dropdown" in target_lower):
            ...
            selectors.extend([
                ...
                {"selector": f"button:has-text('{target}')[class*='border'][aria-haspopup='true']", "strategy": "bordered_dropdown_aria", "priority": 2},
                {"selector": f"button:has-text('{target}')[style*='border'][aria-haspopup='true']", "strategy": "styled_dropdown_aria", "priority": 2},
                ...
                {"selector": f"button:has-text('{target}')[data-toggle='dropdown']:not([role='tab'])", "strategy": "dropdown_toggle", "priority": 3},
                
                # Lower priority: exclude tabs but include buttons
                {"selector": f"button:has-text('{target}'):not([role='tab']):not([class*='tab'])", "strategy": "button_not_tab", "priority": 4},
                {"selector": f"[role='button']:has-text('{target}'):not([role='tab']):not([class*='tab'])", "strategy": "role_button_not_tab", "priority": 5},
            ])
        elif context.get("element_type") == "bordered-button" or context.get("has_border"):
            # Prioritize buttons with visual borders/boundaries
            selectors.extend([
                {"selector": f"button:has-text('{target}')[class*='border']:not([role='tab'])", "strategy": "bordered_button", "priority": 2},
                {"selector": f"button:has-text('{target}')[style*='border']:not([role='tab'])", "strategy": "styled_border_button", "priority": 2},
                {"selector": f"button:has-text('{target}')[class*='outline']:not([role='tab'])", "strategy": "outlined_button", "priority": 2},
                {"selector": f"button:has-text('{target}'):not([role='tab']):not([class*='tab'])", "strategy": "button_not_tab", "priority": 3},
            ])
        elif context.get("element_type") == "button" or "button" in target_lower:
            if context.get("exclude_tabs"):
                ...
                selectors.extend([
                    ...
                    {"selector": f"[role='button']:has-text('{target}'):not([role='tab'])", "strategy": "role_button_not_tab", "priority": 3},
                ])
            else:
                # Regular button matching
                selectors.extend([
                    {"selector": f"button:has-text('{target}')", "strategy": "button_text", "priority": 3},
                    {"selector": f"[role='button']:has-text('{target}')", "strategy": "button_role", "priority": 4},
                ])
        
        # Strategy 3: Link matching
        if context.get("element_type") == "link" or any(word in target_lower for word in ["sign in", "login", "register"]):
            selectors.extend([
        ...
        if context.get("color"):
            color = context["color"]
            selectors.extend([
                {"selector": f"button[class*='{color}']:has-text('{target}')", "strategy": f"{color}_button", "priority": 7},
                {"selector": f"[class*='{color}'][class*='button']:has-text('{target}')", "strategy": f"{color}_class", "priority": 8},
            ])
        ...
        if context.get("position"):
            ...
            if position in ["top-right", "top-right-corner"]:
                ...
                selectors.extend([
                    ...
                    {"selector": f"[class*='top'][class*='right'] button:has-text('{target}')", "strategy": "top_right_specific", "priority": 1},
                    
                    # Header and toolbar areas (most likely for top-right buttons)
                    {"selector": f"header button:has-text('{target}'){left_exclusions}", "strategy": "header_button", "priority": 2},
                    {"selector": f"[class*='header'] button:has-text('{target}'){left_exclusions}", "strategy": "header_class", "priority": 2},
                    {"selector": f"[class*='toolbar'] button:has-text('{target}'){left_exclusions}", "strategy": "toolbar_button", "priority": 2},
                    {"selector": f"[class*='action'] button:has-text('{target}'){left_exclusions}", "strategy": "action_button", "priority": 2},
                    
                    # Exclude tabs and left-side elements completely
                    {"selector": f"button:has-text('{target}'):not([role='tab']):not([class*='tab']){left_exclusions}", "strategy": "button_not_tab_not_left", "priority": 3},
                    
                    # Position-based with strong right-side preference
                    {"selector": f"[class*='right'] button:has-text('{target}'){left_exclusions}", "strategy": "right_positioned", "priority": 4},
                    
                    # Generic but with exclusions
                    {"selector": f"button:has-text('{target}'){left_exclusions}", "strategy": "button_no_left", "priority": 8},
                ])
            else:
                # Generic position matching for other positions
                selectors.extend([
                    {"selector": f"[class*='{position}'] *:has-text('{target}')", "strategy": f"{position}_positioned", "priority": 9},
                ])
        
        # Strategy 6: Enhanced dropdown-specific matching (for menu items)
        if context.get("parent_type") == "dropdown" or "dropdown" in target_lower or "from dropdown" in target_lower:
            selectors.extend([
                # High priority - exact role matching
                {"selector": f"[role='menuitem']:has-text('{target}')", "strategy": "menuitem", "priority": 1},
                {"selector": f"button[role='menuitem']:has-text('{target}')", "strategy": "button_menuitem", "priority": 1},
                {"selector": f"[role='option']:has-text('{target}')", "strategy": "option", "priority": 2},
                
                # Medium priority - structural matching
                {"selector": f"li[role='menuitem']:has-text('{target}')", "strategy": "li_menuitem", "priority": 3},
                {"selector": f"[role='menu'] button:has-text('{target}')", "strategy": "menu_button", "priority": 3},
                {"selector": f"[role='menu'] [role='menuitem']:has-text('{target}')", "strategy": "nested_menuitem", "priority": 3},
                
                # Lower priority - broader matching
                {"selector": f"[role='menu'] *:has-text('{target}')", "strategy": "menu_child", "priority": 5},
                {"selector": f"ul[role='menu'] li:has-text('{target}')", "strategy": "menu_list_item", "priority": 6},
                {"selector": f"div[role='menu'] *:has-text('{target}')", "strategy": "div_menu_child", "priority": 7},
                
                # Fallback - visible dropdown items
                {"selector": f"[aria-expanded='true'] + * [role='menuitem']:has-text('{target}')", "strategy": "expanded_menuitem", "priority": 8},
                {"selector": f"[data-state='open'] [role='menuitem']:has-text('{target}')", "strategy": "open_menuitem", "priority": 8},
            ])
        
        # Strategy 7: Advanced attribute matching
        selectors.extend([
            {"selector": f"[aria-label*='{target}' i]", "strategy": "aria_label", "priority": 10},
            {"selector": f"[title*='{target}' i]", "strategy": "title_attr", "priority": 11},
            {"selector": f"[data-testid*='{target.lower().replace(' ', '-')}']", "strategy": "test_id", "priority": 12},
        ])
        
        # Strategy 8: Generic element matching (lowest priority)
        selectors.extend([
            {"selector": f"*:has-text('{target}')", "strategy": "generic_text", "priority": 15},
        ])
    ...
    def _generate_field_selectors(self, field_description: str, field_type: Optional[str]) -> List[Dict[str, Any]]:
        ...
        if "message" in field_lower or "input" in field_lower or "text" in field_lower:
            selectors.extend([
                {"selector": "input[type='text']:visible", "strategy": "text_input", "priority": 1},
                {"selector": "textarea:visible", "strategy": "textarea", "priority": 1},
                {"selector": "input:not([type='checkbox']):not([type='radio']):not([type='submit']):not([type='button']):visible", "strategy": "generic_input", "priority": 2},
            ])
        ...
        selectors.extend([
            {"selector": f"input[placeholder*='{field_description}' i]:not([type='checkbox']):not([type='radio'])", "strategy": "placeholder", "priority": 3},
        ...
        field_name = field_description.lower().replace(" ", "").replace("field", "")
        selectors.extend([
            {"selector": f"input[name*='{field_name}']:not([type='checkbox']):not([type='radio'])", "strategy": "name_attr", "priority": 4},
            {"selector": f"input[name='{field_name}']:not([type='checkbox']):not([type='radio'])", "strategy": "name_exact", "priority": 4},
        ])
        
        # Strategy 5: ID attribute matching
        selectors.extend([
            {"selector": f"input[id*='{field_name}']:not([type='checkbox']):not([type='radio'])", "strategy": "id_attr", "priority": 5},
            {"selector": f"input[id='{field_name}']:not([type='checkbox']):not([type='radio'])", "strategy": "id_exact", "priority": 5},
        ])
        
        # Strategy 6: Label-based matching
        selectors.extend([
            {"selector": f"input[aria-label*='{field_description}' i]:not([type='checkbox']):not([type='radio'])", "strategy": "aria_label", "priority": 6},
            {"selector": f"label:has-text('{field_description}') + input:not([type='checkbox']):not([type='radio'])", "strategy": "adjacent_label", "priority": 7},
            {"selector": f"label:has-text('{field_description}') input:not([type='checkbox']):not([type='radio'])", "strategy": "nested_label", "priority": 8},
        ])
        
        # Strategy 6: Common field patterns
        if "email" in field_lower:
            selectors.extend([
                {"selector": "input[type='email']", "strategy": "email_type", "priority": 1},
                {"selector": "input[name*='email']", "strategy": "email_name", "priority": 2},
            ])
        
        if "password" in field_lower:
            selectors.extend([
                {"selector": "input[type='password']", "strategy": "password_type", "priority": 1},
                {"selector": "input[name*='password']", "strategy": "password_name", "priority": 2},
            ])
        
        # Strategy 7: Generic input matching (lowest priority)
        selectors.extend([
            {"selector": "input", "strategy": "generic_input", "priority": 20},
            {"selector": "textarea", "strategy": "generic_textarea", "priority": 21},
        ])
    ...
    async def find_ui_panel(self, page: Page, panel_type: str) -> Optional[Locator]:
    ...
    def _generate_panel_selectors(self, panel_type: str) -> List[Dict[str, Any]]:
        ...
        if "left" in panel_lower:
            selectors.extend([
                ...
                {"selector": "[class*='sidebar']", "strategy": "sidebar_class"},
                {"selector": "[class*='nav']", "strategy": "nav_class"},
                {"selector": "[class*='left']", "strategy": "left_class"},
                {"selector": "[class*='file']", "strategy": "file_class"},
                {"selector": "[data-testid*='sidebar']", "strategy": "sidebar_testid"},
                {"selector": "[data-testid*='nav']", "strategy": "nav_testid"},
                {"selector": "[data-testid*='left']", "strategy": "left_testid"},
                {"selector": "[role='navigation']", "strategy": "nav_role"},
                # Structure-based (first child of main container)
                {"selector": "main > div:first-child", "strategy": "main_first_child"},
                {"selector": "[class*='container'] > div:first-child", "strategy": "container_first_child"},
            ])
        
        elif "center" in panel_lower:
            selectors.extend([
                # Document/content panels
                {"selector": "[class*='content']", "strategy": "content_class"},
                {"selector": "[class*='center']", "strategy": "center_class"},
                {"selector": "[class*='main']", "strategy": "main_class"},
                {"selector": "[class*='document']", "strategy": "document_class"},
                {"selector": "[class*='preview']", "strategy": "preview_class"},
                {"selector": "[data-testid*='content']", "strategy": "content_testid"},
                {"selector": "[data-testid*='center']", "strategy": "center_testid"},
                {"selector": "[data-testid*='main']", "strategy": "main_testid"},
                # Upload areas
                {"selector": "[class*='upload']", "strategy": "upload_class"},
                {"selector": "[class*='drop']", "strategy": "drop_class"},
                {"selector": "text='NO FILES ADDED YET'", "strategy": "upload_text"},
                {"selector": "text='Drop anywhere to upload'", "strategy": "drop_text"},
                # Structure-based (middle child)
                {"selector": "main > div:nth-child(2)", "strategy": "main_middle_child"},
                {"selector": "[class*='container'] > div:nth-child(2)", "strategy": "container_middle_child"},
            ])
        
        elif "right" in panel_lower:
            selectors.extend([
                # Chat/conversation panels
                {"selector": "[class*='chat']", "strategy": "chat_class"},
                {"selector": "[class*='conversation']", "strategy": "conversation_class"},
                {"selector": "[class*='right']", "strategy": "right_class"},
                {"selector": "[class*='message']", "strategy": "message_class"},
                {"selector": "[data-testid*='chat']", "strategy": "chat_testid"},
                {"selector": "[data-testid*='conversation']", "strategy": "conversation_testid"},
                {"selector": "[data-testid*='right']", "strategy": "right_testid"},
                {"selector": "[data-testid*='message']", "strategy": "message_testid"},
                # Sample prompts areas
                {"selector": "text='Sample Prompts'", "strategy": "sample_prompts_text"},
                {"selector": "[class*='prompt']", "strategy": "prompt_class"},
                # Structure-based (last child)
                {"selector": "main > div:last-child", "strategy": "main_last_child"},
                {"selector": "[class*='container'] > div:last-child", "strategy": "container_last_child"},
            ])
        
        # Generic panel selectors (lowest priority)
        selectors.extend([
            {"selector": f"[class*='{panel_lower}']", "strategy": f"{panel_lower}_generic_class"},
            {"selector": f"[data-testid*='{panel_lower}']", "strategy": f"{panel_lower}_generic_testid"},
            {"selector": f"*:has-text('{panel_type}')", "strategy": f"{panel_lower}_text_match"},
        ])
