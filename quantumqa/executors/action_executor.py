...
class ActionExecutor:
    ...
    def __init__(self, credentials_file: Path = None):
        ...
        if credentials_file and credentials_file.exists():
            try:
                ...
                print("⚠️ Credential manager not available (cryptography package not installed)")
    ...
    async def navigate(self, page: Page, url: str, options: Dict[str, Any] = None) -> bool:
    ...
    async def click(self, element: Locator, options: Dict[str, Any] = None) -> bool:
        ...
        try:
            ...
            if "dropdown" in options.get("target", "").lower():
                ...
                success = await self._try_dropdown_click_strategies(element, options)
            else:
            ...
            if success:
                ...
                if self._is_dropdown_trigger(options.get("target", "")):
                    ...
                    dropdown_opened = await self._verify_dropdown_opened_strict(page)
                    
                    if dropdown_opened:
                        print(f"    ✅ Dropdown successfully opened and contains menu items")
                    else:
                        print(f"    ❌ CRITICAL: Dropdown did not open properly")
                        print(f"    🔄 Trying alternative dropdown trigger methods...")
                        
                        # Try alternative clicking strategies for dropdown
                        alt_success = await self._try_alternative_dropdown_trigger(element, page)
                        if alt_success:
                            # Re-verify strict opening
                            final_check = await self._verify_dropdown_opened_strict(page)
                            if final_check:
                                print(f"    ✅ Alternative method successfully opened dropdown")
                            else:
                                print(f"    ❌ FAILED: Could not open dropdown with any method")
                                return False
                        else:
                            print(f"    ❌ FAILED: Could not open dropdown with any method")
    ...
    async def type_text(self, element: Locator, text: str, options: Dict[str, Any] = None) -> bool:
        ...
        try:
            ...
            if resolved_text != text:
                ...
                print(f"    ⌨️ Typing: '[CREDENTIAL:{len(resolved_text)} chars]'")
            else:
            ...
            if current_value == resolved_text:
                ...
                print(f"    ⚠️ Input mismatch. Expected: {len(resolved_text)} chars, Got: {len(current_value)} chars")
                return False
    ...
    async def upload_file(self, page: Page, file_path: str) -> bool:
        ...
        try:
            ...
            if input_count > 0:
                print(f"    🎯 Found {input_count} file input(s), using first one")
            ...
            for selector in upload_selectors:
                try:
                    ...
                    if await element.is_visible():
                        ...
                        print(f"    ⚠️ Upload area clicked, but file dialog requires manual interaction")
                        return True
                except:
                    continue
    ...
    async def verify(self, page: Page, verification_type: str, expected_value: Optional[str] = None, options: Dict[str, Any] = None) -> bool:
        ...
        if (verification_type == "page_load" and 
            navigation_context.get("current_page_context") == "navigation_failed"):
            print(f"    🚨 WARNING: Previous navigation failed (Step {navigation_context.get('last_navigation_step')})")
            print(f"    🚨 This page load verification may be checking the WRONG page!")
        ...
        try:
            ...
            if verification_type == "page_load":
                ...
                return await self._verify_element_visible(page, expected_value, timeout)
                ...
                return await self._verify_contains_elements(page, expected_value, timeout)
            
            elif verification_type == "process_started":
                return await self._verify_process_started(page, expected_value, timeout)
            
            elif verification_type == "creation_completed":
                return await self._verify_creation_completed(page, expected_value, timeout)
            
            elif verification_type == "url_redirect_with_patterns":
                return await self._verify_url_redirect_with_patterns(page, options)
    ...
    async def wait(self, page: Page, wait_type: str, duration: int = 2) -> bool:
        ...
        try:
            if wait_type == "time":
                ...
                await page.wait_for_load_state("domcontentloaded", timeout=duration * 1000)
    ...
    async def _try_click_strategies(self, element: Locator, options: Dict[str, Any]) -> bool:
        ...
        if element_info.get("is_send_button") or element_info.get("has_overlay_interference"):
            strategies = [
            ...
            print(f"      🎯 Using optimized strategy order for send/overlay elements")
    ...
    async def _analyze_element(self, element: Locator) -> Dict[str, bool]:
        ...
        try:
            ...
            analysis = {
                "is_send_button": "send" in test_id.lower() and tag_name == "svg",
            ...
            try:
                page = element.page
                overlay_count = await page.locator("[data-pollen-overlay='true']").count()
                if overlay_count > 0:
            ...
            return {"is_send_button": False, "has_overlay_interference": False, "is_disabled": False, "needs_force": False}
    
    async def _try_dropdown_click_strategies(self, element: Locator, options: Dict[str, Any]) -> bool:
    ...
    async def _dropdown_force_click(self, element: Locator, options: Dict[str, Any]) -> bool:
    ...
    async def _dropdown_js_click(self, element: Locator, options: Dict[str, Any]) -> bool:
    ...
    async def _dropdown_parent_click(self, element: Locator, options: Dict[str, Any]) -> bool:
            ...
            for parent_selector in parent_selectors:
                try:
                    ...
                    if await parent.is_visible() and await parent.is_enabled():
                        ...
                        parent_cursor = await parent.evaluate("el => getComputedStyle(el).cursor")
                        parent_role = await parent.get_attribute("role")
                        
                        if parent_cursor == "pointer" or parent_role in ["menuitem", "button", "option"]:
                            print(f"        📍 Trying clickable parent with cursor: {parent_cursor}, role: {parent_role}")
    ...
    async def _dropdown_coordinate_click(self, element: Locator, options: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if box:
                ...
                click_points = [
                    (box["x"] + box["width"] * 0.5, box["y"] + box["height"] * 0.5),  # Center
                    (box["x"] + box["width"] * 0.3, box["y"] + box["height"] * 0.5),  # Left center
                    (box["x"] + box["width"] * 0.7, box["y"] + box["height"] * 0.5),  # Right center
                ]
    ...
    async def _dropdown_keyboard_select(self, element: Locator, options: Dict[str, Any]) -> bool:
    ...
    async def _regular_click(self, element: Locator, options: Dict[str, Any]) -> bool:
        """Regular click strategy."""
        await element.click(timeout=options.get("timeout", 5000))
        return True
    
    async def _force_click(self, element: Locator, options: Dict[str, Any]) -> bool:
        """Force click strategy for difficult elements."""
        await element.click(force=True, timeout=options.get("timeout", 5000))
        return True
    
    async def _javascript_click(self, element: Locator, options: Dict[str, Any]) -> bool:
        """JavaScript click strategy."""
        await element.evaluate("el => el.click()")
        return True
    
    async def _center_click(self, element: Locator, options: Dict[str, Any]) -> bool:
    ...
    async def _verify_page_load(self, page: Page, navigation_context: Dict[str, Any] = None) -> bool:
        ...
        # 🧠 CONTEXT-AWARE LOGIC
        if navigation_context.get("current_page_context") == "navigation_failed":
            last_nav_step = navigation_context.get("last_navigation_step")
            expected_nav_info = navigation_context.get("expected_navigation_info", {})
            pattern_type = expected_nav_info.get("pattern_type", "unknown")
            
            print(f"    🚨 CONTEXT-AWARE ANALYSIS:")
            print(f"       → Previous navigation failed in Step {last_nav_step}")
            ...
            if pattern_type == "prefix_suffix":
                ...
                print(f"       → Expected URL with prefix: '{expected_prefix}' and suffix: '{expected_suffix}'")
                
                if expected_prefix and expected_prefix.lower() not in current_url.lower():
                    print(f"    ❌ CONTEXT FAILURE: Page loaded, but we're on the WRONG page!")
                    print(f"       → Missing expected URL prefix: '{expected_prefix}'")
                    print(f"       → Basic page load: {basic_success}, but context says: WRONG PAGE")
                    return False
                ...
                print(f"    ❌ CONTEXT FAILURE: Page loaded, but expected navigation did NOT occur!")
                print(f"       → Navigation was expected: '{expected_desc}'")
                print(f"       → Basic page load: {basic_success}, but context says: NO NAVIGATION HAPPENED")
                return False
            
            else:
                # Unknown pattern type - fallback to basic logic
                print(f"       → Unknown navigation pattern type: {pattern_type}")
                print(f"    ✅ Basic page load: {basic_success}")
                return basic_success
        else:
            # Normal page load verification when no navigation context issues
            print(f"    {'✅' if basic_success else '❌'} Page load check: {basic_success}")
            return basic_success
    
    async def _verify_title_contains(self, page: Page, expected_text: str) -> bool:
    ...
    async def _verify_url_change(self, page: Page, expected_pattern: str) -> bool:
        ...
        url_suggests_navigation = any(keyword in current_url.lower() for keyword in [
            'converse', 'chat', 'conversation', 'apps', 'edit', 'create', 'new'
        ])
        ...
        context_match = False
        if context_keywords:
            url_context_match = any(keyword in current_url.lower() for keyword in context_keywords)
            title_context_match = any(keyword in current_title.lower() for keyword in context_keywords)
            context_match = url_context_match or title_context_match
            print(f"    🎯 Context keywords: {context_keywords}")
            print(f"       → URL context match: {'✅' if url_context_match else '❌'}")
            print(f"       → Title context match: {'✅' if title_context_match else '❌'}")
        else:
        ...
        print(f"    🔍 Navigation occurred: {'✅' if url_suggests_navigation else '❌'}")
        print(f"    🎯 Context match: {'✅' if context_match else '❌'}")
        print(f"    {'✅' if success else '❌'} Smart URL verification: {success}")
        
        return success
    
    async def _verify_element_visible(self, page: Page, element_description: str, timeout: int) -> bool:
        ...
        try:
            ...
            if any(panel in element_lower for panel in ["left panel", "center panel", "right panel"]):
                panel_type = None
                ...
                if panel_type:
                    ...
                    panel_element = await element_finder.find_ui_panel(page, panel_type)
                    
                    if panel_element:
                        print(f"    ✅ Found {panel_type} using enhanced panel detection")
                        return True
                    else:
            ...
            search_strategies = [
                ...
                *[f"*:has-text('{word}')" for word in element_description.split() if len(word) > 2],
            ]
                ...
                try:
                    ...
                    if count > 0:
                        ...
                        for i in range(min(count, 3)):  # Check up to 3 elements
                            ...
                            if await element.is_visible(timeout=timeout // len(search_strategies)):
                                element_text = await element.text_content()
                                print(f"    ✅ Found visible element: '{element_text[:50]}...' using {strategy}")
                                return True
                except Exception:
                    continue
    ...
    async def _verify_contains_elements(self, page: Page, expected_elements: str, timeout: int) -> bool:
        ...
        try:
            ...
            if ',' in expected_elements:
                elements_to_check = [elem.strip() for elem in expected_elements.split(',')]
            else:
            ...
            for element_name in elements_to_check:
                ...
                for selector in selectors:
                    try:
                        element = page.locator(selector).first
                        if await element.is_visible(timeout=timeout // len(elements_to_check)):
            ...
            success = found_count >= len(elements_to_check) * 0.5  # At least 50% found
            print(f"    📊 Found {found_count}/{len(elements_to_check)} elements")
            print(f"    {'✅' if success else '❌'} Contains elements verification: {success}")
    ...
    async def _verify_modal_appears(self, page: Page, timeout: int) -> bool:
        ...
        try:
            ...
            for selector in modal_selectors:
                try:
                    element = page.locator(selector).first
                    if await element.is_visible(timeout=timeout / len(modal_selectors)):
    ...
    def _is_dropdown_trigger(self, target: str) -> bool:
        ...
        dropdown_indicators = [
            "dropdown", "with dropdown", "dropdown arrow", 
            "create button", "dropdown button"
        ]
        return any(indicator in target_lower for indicator in dropdown_indicators)
    ...
    async def _verify_dropdown_opened_strict(self, page: Page) -> bool:
        ...
        try:
            print(f"    🔍 Strict dropdown validation: Looking for menu items...")
            ...
            expected_items = ["conversation", "chatbot", "app"]  # Known AIHub dropdown items
            ...
            for selector in dropdown_item_selectors:
                try:
                    ...
                    if count > 0:
                        print(f"    🎯 Found {count} menu items using: {selector}")
                        ...
                        for i in range(min(count, 5)):  # Check up to 5 items
                            ...
                            if await element.is_visible(timeout=1000):
                                ...
                                if any(item in text_lower for item in expected_items):
                                    print(f"    ✅ Found expected dropdown item: '{text}'")
            ...
            if menu_items_found:
                print(f"    ✅ Strict validation passed: Dropdown is open with menu items")
                return True
            else:
                print(f"    ❌ Strict validation failed: No recognizable menu items found")
                return False
    ...
    async def _verify_process_started(self, page: Page, process_name: str, timeout: int) -> bool:
        ...
        try:
            ...
            for i, strategy in enumerate(verification_strategies):
                try:
                    ...
                    if result:  # If any strategy succeeds, consider process started
                        print(f"    ✅ Process verification successful via strategy {i+1}")
                        return True
                except Exception as e:
            ...
            print(f"    📈 Process verification: {success_count}/{len(results)} strategies succeeded")
            
            if success_count == 0:
                print(f"    ❌ CRITICAL: No evidence that '{process_name}' process started")
                print(f"    💡 This appears to be an application issue, not a test issue")
                return False
    ...
    async def _verify_creation_completed(self, page: Page, item_name: str, timeout: int) -> bool:
        ...
        try:
            ...
            for i, strategy in enumerate(completion_strategies):
                try:
                    result = await strategy
                    results.append(result)
                    print(f"    📊 Completion strategy {i+1}: {'✅' if result else '❌'}")
                    if result:
                        print(f"    ✅ Creation completed verification successful")
                        return True
                except Exception as e:
            ...
            success_count = sum(results)
            print(f"    📈 Creation verification: {success_count}/{len(results)} strategies succeeded")
    ...
    async def _check_url_change_for_process(self, page: Page, process_name: str) -> bool:
        """Check if URL changed to indicate process started."""
        current_url = page.url
        process_keywords = ["create", "new", "setup", "wizard", process_name.lower()]
        
        for keyword in process_keywords:
            if keyword in current_url.lower():
                print(f"    🌐 URL indicates process started: contains '{keyword}'")
                return True
        
        print(f"    🌐 URL doesn't indicate process start: {current_url}")
        return False
    
    async def _check_process_ui_elements(self, page: Page, process_name: str, timeout: int) -> bool:
        ...
        process_ui_selectors = [
            ...
            ".setup-wizard", ".configuration-form", ".creation-wizard",
            # Form elements that suggest process started
            "input[placeholder*='name']", "input[placeholder*='title']",
            "textarea[placeholder*='description']",
            # Specific chatbot indicators
            "*:has-text('chatbot name')", "*:has-text('bot configuration')",
            "*:has-text('chatbot settings')", "*:has-text('configure')"
        ]
        
        for selector in process_ui_selectors:
            try:
                element = page.locator(selector).first
                if await element.is_visible(timeout=timeout // len(process_ui_selectors)):
                    element_text = await element.text_content() or ""
                    print(f"    🎯 Process UI detected: '{element_text[:50]}...' using {selector}")
                    return True
            except Exception:
                continue
    ...
    async def _check_title_change_for_process(self, page: Page, process_name: str) -> bool:
        """Check if page title indicates process started."""
        current_title = await page.title()
        process_keywords = ["create", "new", "setup", "configure", process_name.lower()]
        
        for keyword in process_keywords:
            if keyword in current_title.lower():
                print(f"    📄 Title indicates process started: contains '{keyword}' in '{current_title}'")
                return True
        
        print(f"    📄 Title doesn't indicate process start: '{current_title}'")
        return False
    
    async def _check_process_forms(self, page: Page, process_name: str, timeout: int) -> bool:
        """Check for forms/modals related to process."""
        form_selectors = [
            "form[role='dialog']", "[role='dialog'] form", 
            ".modal form", ".dialog form",
            f"form[data-testid*='{process_name.lower()}']",
        ...
        for selector in form_selectors:
            try:
                element = page.locator(selector).first
                if await element.is_visible(timeout=timeout // len(form_selectors)):
                    form_content = await element.text_content() or ""
                    if process_name.lower() in form_content.lower() or any(kw in form_content.lower() for kw in ["name", "title", "description", "configure"]):
                        print(f"    📝 Process form detected with relevant content")
                        return True
            except Exception:
                continue
    ...
    async def _check_process_indicators(self, page: Page, timeout: int) -> bool:
        """Check for loading/progress indicators."""
        indicator_selectors = [
            ".loading", ".spinner", ".progress",
            "[aria-label*='loading']", "[aria-label*='progress']",
            "*:has-text('loading')", "*:has-text('creating')",
            "*:has-text('setting up')", "*:has-text('configuring')"
        ]
    ...
    async def _check_new_item_created(self, page: Page, item_name: str, timeout: int) -> bool:
        """Check if new item appears in lists/grids."""
        item_selectors = [
            f"*:has-text('{item_name}')",
            "[role='listitem']", ".list-item", ".grid-item", 
            ".card", ".tile", "tr", "li"
        ]
                    ...
                    for i in range(min(count, 5)):  # Check first 5 items
                        ...
                        if await element.is_visible(timeout=timeout // 10):
                            ...
                            if item_name.lower() in text.lower():
                                print(f"    🎯 New item found: '{text[:50]}...'")
                                return True
            except Exception:
                continue
    ...
    async def _check_success_notification(self, page: Page, timeout: int) -> bool:
        ...
        for selector in notification_selectors:
            try:
                element = page.locator(selector).first
                if await element.is_visible(timeout=timeout // len(notification_selectors)):
                    notification_text = await element.text_content() or ""
                    print(f"    🎉 Success notification: '{notification_text[:50]}...'")
                    return True
            except Exception:
                continue
    ...
    async def _check_redirect_to_new_item(self, page: Page, item_name: str) -> bool:
        """Check if redirected to new item page."""
        current_url = page.url
        redirect_indicators = [item_name.lower(), "edit", "configure", "settings"]
        
        for indicator in redirect_indicators:
            if indicator in current_url.lower():
                print(f"    🔄 Redirected to new item page: contains '{indicator}'")
                return True
        
        return False
    
    async def _check_creation_confirmation(self, page: Page, timeout: int) -> bool:
        """Check for creation confirmation dialogs."""
        confirmation_selectors = [
            "[role='dialog']:has-text('created')",
            "[role='dialog']:has-text('success')",
            ".modal:has-text('created')",
            ".dialog:has-text('success')"
        ]
        
        for selector in confirmation_selectors:
            try:
                element = page.locator(selector).first
                if await element.is_visible(timeout=timeout // len(confirmation_selectors)):
    ...
    async def _verify_url_redirect_with_patterns(self, page: Page, options: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            prefix_match = url_prefix.lower() in current_url.lower() if url_prefix else True
            
            # Check if URL contains the expected suffix  
            suffix_match = url_suffix.lower() in current_url.lower() if url_suffix else True
            ...
            if success:
                ...
                print(f"       Expected URL to contain prefix '{url_prefix}' and suffix '{url_suffix}'")
                if not prefix_match:
                    print(f"       Missing prefix: '{url_prefix}' not found in URL")
                if not suffix_match:
                    print(f"       Missing suffix: '{url_suffix}' not found in URL")
    ...
    async def _try_alternative_dropdown_trigger(self, element: Locator, page: Page) -> bool:
