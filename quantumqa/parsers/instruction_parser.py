...
class InstructionParser:
    ...
    async def parse(self, instruction: str) -> Dict[str, Any]:
        ...
        if instruction.startswith('#') or instruction.startswith('//') or not instruction:
            return {
                "action": "comment",
    ...
    def _build_action_plan(self, action_type: str, instruction: str, match: re.Match, extractor: Dict) -> Dict[str, Any]:
        ...
        if action_type == "navigate":
            plan.update(self._extract_navigation_params(instruction, match, extractor))
        
        elif action_type == "click":
            plan.update(self._extract_click_params(instruction, match, extractor))
        
        elif action_type == "type":
            plan.update(self._extract_type_params(instruction, match, extractor))
        
        elif action_type == "upload":
            plan.update(self._extract_upload_params(instruction, match, extractor))
        
        elif action_type in ["send", "select", "explore", "test", "confirm", "close"]:
            plan.update(self._extract_generic_target_params(instruction, match, extractor))
        
        elif action_type == "verify":
            plan.update(self._extract_verify_params(instruction, match, extractor))
        
        elif action_type == "wait":
            plan.update(self._extract_wait_params(instruction, match, extractor))
        
        return plan
    
    def _extract_navigation_params(self, instruction: str, match: re.Match, extractor: Dict) -> Dict[str, Any]:
    ...
    def _extract_type_params(self, instruction: str, match: re.Match, extractor: Dict) -> Dict[str, Any]:
        ...
        if type_match:
            text = type_match.group(1).strip()  # This preserves the original case
    ...
    def _extract_upload_params(self, instruction: str, match: re.Match, extractor: Dict) -> Dict[str, Any]:
    ...
    def _extract_generic_target_params(self, instruction: str, match: re.Match, extractor: Dict) -> Dict[str, Any]:
        ...
        if extractor.get("target"):
            ...
            elif isinstance(target_ref, int) and target_ref <= len(match.groups()):
                target = match.group(target_ref)  # Regex group reference
        
        return {
            "target": target,
            "action_context": instruction.lower()
        }
    
    def _extract_verify_params(self, instruction: str, match: re.Match, extractor: Dict) -> Dict[str, Any]:
        ...
        if "expected_value" in extractor and match and match.groups():
            ...
            if isinstance(expected_value_index, int) and len(match.groups()) >= expected_value_index:
        ...
        if verification_type == "url_redirect_with_patterns" and match and match.groups():
            if "url_prefix" in extractor and len(match.groups()) >= extractor["url_prefix"]:
                verification_options["url_prefix"] = match.group(extractor["url_prefix"]).strip()
            if "url_suffix" in extractor and len(match.groups()) >= extractor["url_suffix"]:
                verification_options["url_suffix"] = match.group(extractor["url_suffix"]).strip()
        ...
        if verification_type == "generic":
            if "loads successfully" in instruction_lower:
                ...
                title_match = re.search(r'title contains\s+["\']?([^"\']+)["\']?', instruction_lower)
                if title_match:
                ...
                url_match = re.search(r'(?:url|page).*?(?:change|contain).*?["\']?([^"\']+)["\']?', instruction_lower)
                if url_match:
                ...
                if not expected_value:
                    ...
                    key_elements = ['button', 'dropdown', 'tab', 'panel', 'menu', 'create', 'purple', 'deploy', 'review', 'data']
                    found_elements = [elem for elem in key_elements if elem in instruction_lower]
                    if found_elements:
                        expected_value = ' '.join(found_elements)
            elif "has" in instruction_lower and ("button" in instruction_lower or "option" in instruction_lower):
                verification_type = "contains_elements"
                # Extract what should be contained
                contains_match = re.search(r'has.*?(?:button|option).*?for\s+([^"\']+)', instruction_lower)
                if contains_match:
    ...
    def _extract_wait_params(self, instruction: str, match: re.Match, extractor: Dict) -> Dict[str, Any]:
    ...
    def _extract_context_clues(self, instruction: str) -> Dict[str, Any]:
        ...
        if ("dropdown arrow" in instruction_lower or "with dropdown" in instruction_lower or 
            ("border" in instruction_lower and "dropdown" in instruction_lower)):
            ...
            context["requires_force"] = True  # Dropdown items often need force click
        ...
        if "from dropdown" in instruction_lower:
            ...
            dropdown_match = re.search(r'(\w+)\s+(?:button|option)?\s*from\s+dropdown', instruction_lower)
            if dropdown_match:
    ...
    def _load_context_hints(self) -> Dict[str, Any]:
        ...
        default_hints = {
            ...
            "element_types": ["button", "link", "dropdown", "tab", "input", "field"],
